...
<div class="wrap">
  ...
  <form action="<?php echo esc_url( admin_url( 'admin.php?page=alike_settings' ) ) ?>" method="post">
    ...
    <table class="form-table">
      <tbody>
        <tr>
          <th><?php esc_html_e('Alike preview page', 'alike') ?></th>
          <td>
            <select class="widefat" name="preview_page">
              <?php $pages = get_pages(); foreach ($pages as $page) { ?>
                <option <?php echo ( isset($alike_settings['preview_page']) && $alike_settings['preview_page'] == $page->ID ) ? 'selected="selected"': '' ?>value="<?php echo esc_attr( $page->ID ) ?>"><?php echo esc_attr( $page->post_title ) ?></option>
              <?php } ?>
